...
import { CalendarClock, MessageSquare, Check, RotateCcw, Clock, Package, Store as StoreIcon, Camera, Flag } from "lucide-react";
          ...
          {(issue.status === "open" || issue.status === "rectifying") && (
            <>
              ...
              <div className="flex items-center gap-2 mt-2">
                <button className="btn-subtle text-xs"><Camera className="h-3.5 w-3.5" /> 上传整改证据</button>
                <button
                  onClick={() => { if (note.trim()) { submitRectification(issue.id, note); setNote(""); } }}
                  disabled={!note.trim()}
